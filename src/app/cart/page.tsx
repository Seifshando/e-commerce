...
setnumberOfCartItem(response.data.products.reduce((acc:number, p:any)  => acc + p.count, 0));
}
} catch (error) {
...
setTotal(response.data.totalCartPrice);
setnumberOfCartItem(
response.data.products.reduce((acc: number, p: any) => acc + p.count, 0)
);
...
toast.error(error.message || "Something went wrong removing product", {
position: "top-center",
});
} finally {
...
// تحديث الكمية
// تحديث الكمية مع منطق جديد
async function updateQuantity(id: string, newCount: number) {
setProcessingIds((prev) => [...prev, id]);

try {
// لو الكمية الجديدة أقل من 1 -> اعمل removeItem
if (newCount < 1) {
    await removeItem(id);
    return;
}
...
toast.error(error.message || "Something went wrong updating quantity", {
    position: "top-center",
});
} finally {
setProcessingIds((prev) => prev.filter((pid) => pid !== id));
...
toast.error("Something went wrong clearing cart", { position: "top-center" });
}
}
...
<h1 className="font-bold text-3xl text-center text-red-500 my-10">No Items In Cart</h1>
)}
</>
);
}
